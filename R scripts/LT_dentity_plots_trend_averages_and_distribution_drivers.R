...
# Create plot
tiff(filename = "Plots/LT_slopeDistributions_Drivers.tiff", width = 12, height = 10, units = 'in', res = 600, compression = 'lzw')
...
legend(x=5, y=(4/4*(b-a)+a), legend=expression(paste("-1.35%",y^-1, sep = "")), bty="n", cex=1.3)
...
polygon(x=c(stand_temp$Q10, stand_temp$Q10, stand_temp$Q90, stand_temp$Q90),
        y=c((yy-yy/6),(yy+yy/6),(yy+yy/6),(yy-yy/6)),
        col = 1,border = 0,lwd =1)
legend(x=-18, y=(2.7/4*(b-a)+a), legend=("b, Temperature (°C)"), bty="n", cex=1.3)
legend(x=5, y=(2.7/4*(b-a)+a), legend=expression(paste("+0.57% ",y^-1, sep = "")), bty="n", cex=1.3)
...
polygon(x=c(stand_pH$Q10, stand_pH$Q10, stand_pH$Q90, stand_pH$Q90),
        y=c((yy-yy/6),(yy+yy/6),(yy+yy/6),(yy-yy/6)),
        col = 1,border = 0,lwd =1)
legend(x=-18, y=(1.4/4*(b-a)+a), legend=("c, pH"), bty="n", cex=1.3)
legend(x=5, y=(1.4/4*(b-a)+a), legend=expression(paste("+0.18% ",y^-1, sep = "")), bty="n", cex=1.3)
...
polygon(x=c(stand_o2_dis$Q10, stand_o2_dis$Q10, stand_o2_dis$Q90, stand_o2_dis$Q90),
        y=c((yy-yy/6),(yy+yy/6),(yy+yy/6),(yy-yy/6)),
        col = 1,border = 0,lwd = 1)
legend(x=-28, y=(4/4*(b-a)+a), legend=expression(paste("d, Oxygen dis. (",mg, sep = "", L^-1,")")), bty="n", cex=1.3)
legend(x=2.5, y=(4/4*(b-a)+a), legend=expression(paste("+1.39%",y^-1, sep = "")), bty="n", cex=1.3)
...
polygon(x=c(stand_NH4.N$Q10, stand_NH4.N$Q10, stand_NH4.N$Q90, stand_NH4.N$Q90),
        y=c((yy-yy/6),(yy+yy/6),(yy+yy/6),(yy-yy/6)),
        col = 1,border = 0,lwd =1)
legend(x=-28, y=(2.7/4*(b-a)+a), legend=expression(paste("e, Ammonium (",mg, sep = "", L^-1,")")), bty="n", cex=1.3)
legend(x=2.5, y=(2.7/4*(b-a)+a), legend=expression(paste("-3.89% ",y^-1, sep = "")), bty="n", cex=1.3)
...
# percChange_perYr <- (10^PC_axis1-3.5)*100
# d <- density(percChange_perYr)
# a <- (max(d$y)+(max(d$y)/10))*0
# b <- (max(d$y)+(max(d$y)/10))*3
# par(new=TRUE)
...
# stand_PC_axis1 <- lapply(PC_axis1_Est[,2:11], function(x) (10^x-3.5)*100) # this must be the same as how you calculated the percChange_perYr
# yy <- (0.3/4*(b-a)+a)
...
#         y=c((yy-yy/6),(yy+yy/6),(yy+yy/6),(yy-yy/6)),
#         col = 1,border = 0,lwd =1)
# legend(x=-18, y=(1.4/4*(b-a)+a), legend=("e, Nutrients PC axis 1"), bty="n", cex=1.3)
# legend(x=5, y=(1.4/4*(b-a)+a), legend=expression(paste("-249.34% ",y^-1, sep = "")), bty="n", cex=1.3)
